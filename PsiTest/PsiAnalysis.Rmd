...
library(mosaic)
library(pander)
filePath = "C:\\Users\\jhok2013\\Documents\\PsiTest\\data.csv"
rawDat = read_csv(filePath)

averageDat = rawDat %>% 
  arrange(startDice) %>% 
  mutate(dSize = as.character(paste0("d", startDice, sep = ""))) %>%  
  group_by(dSize, startDice, refillBool) %>% 
  summarise(avg = round(mean(runs),2)) %>% 
  mutate(refillBool = if_else(refillBool == 0, "Without Refill", "With Refill"))

dat = rawDat %>% 
  arrange(startDice) %>% 
  mutate(dSize = as.character(paste0("d", startDice, sep = "")),
         refillBool = if_else(refillBool == 0, "Without Refill", "With Refill"))
```
...
```{r dataTable1, message=FALSE, warning=FALSE}
dat %>% 
